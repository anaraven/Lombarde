...
expl.paths <- unique(unlist(mclapply(1:N, path.extremes, shared_pred, coexps),
			   recursive=FALSE, use.names=FALSE))
names(expl.paths) <- sapply(expl.paths, paste, collapse=" ")
print(paste("number of extremes:",length(expl.paths)))
# count the number of non-trivial paths
non.trivial <- sapply(expl.paths, function(e) e[1]!=e[2])
print(table(non.trivial))

# replace each pair (a,b) for a list of all short-path-a-b
expl.paths <- mcmapply(function(e) {
  get.all.shortest.paths(g, e[1], e[2], mode="out")$res}, expl.paths[non.trivial])
...
npath <- table(sapply(expl.paths, length))
...
if(!is.null(out.file)) {
  all.edges <- lapply(unlist(expl.paths[non.trivial], recursive = F),
...
if(!is.null(asp.file)) {
  cat("n.obs",N,"\n", file=asp.file)
  vid <- 1
  for(extremes in names(expl.paths)) {
    for(l in 1:length(expl.paths[[extremes]])) {
      edgelist <- as.numeric(E(g, path=expl.paths[[extremes]][[l]]))
  ...
  for(i in 1:N){
    ...
    for(r in shared_pred[[i]]){
      ...
      for(j in expl.paths[[paste(r,a)]]) {
        cat("path\t",r,"\t",a,"\tp",j,"\n", sep="",file=asp.file, append=TRUE)
      }
      for(j in expl.paths[[paste(r,b)]]) {
        cat("path\t",r,"\t",b,"\tp",j,"\n", sep="",file=asp.file, append=TRUE)
      }
    }
  }
